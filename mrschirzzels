...
-- Hide if surrounded by opponents
function hide()
    ...
    if surroundingOpponents > 2 then
        print(colors.purple .. "Surrounded by opponents, hiding..." .. colors.reset)
...
function decideNextAction()
    if Me.energy < 0.3 then
        hide()
    elseif not attackStrongestOpponent() and not attackWeakestOpponent() then
        moveToStrategicPosition()
    end
end
